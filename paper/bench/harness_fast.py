...
SYSTEMS = {
    ...
    "edge": {
        "url": "https://models-edge.mlpal.ai/v1/messages",
        "headers": {"Authorization": f"Bearer {os.environ.get('MLPAL_PROD_KEY', '')}"},
    },
    "litellm_full": {
        "url": "http://localhost:4001/v1/messages",
        "headers": {"x-api-key": "<your-virtual-key>", "anthropic-version": "2023-06-01"},
    },
    "fake": {
...
def openai_body(body: dict, model: str) -> dict:
    return {
        "model": model,
...
def one_stream(url: str, headers: dict, body: dict, wire: str = "anthropic") -> dict:
...
def run(n_trials: int, systems: list[str], outfile: str) -> None:
    ...
    for s in systems:
        sysd = SYSTEMS[s]
        b = openai_body(body, sysd["model"]) if sysd.get("wire") == "openai" else body
        try:
            one_stream(sysd["url"], sysd["headers"], b, sysd.get("wire", "anthropic"))
    ...
    for i in range(n_trials):
        ...
        for s in order:
            ...
            b = openai_body(body, sysd["model"]) if sysd.get("wire") == "openai" else body
            try:
                m = one_stream(sysd["url"], sysd["headers"], b, sysd.get("wire", "anthropic"))
                results[s].append(m)
